...
from astrocast.cli_interfaces import climage, convert_input, delete_h5_dataset, denoise, detect_events, export_video, \
    motion_correction, move_h5_dataset, subtract_delta, train_denoiser, view_data, view_detection_results, visualize_h5
...
class TestCliSubtractDelta:
    ...
    def setup_class(cls):
        ...
        with h5.File(temp_file.as_posix(), "a") as f:
            f.create_dataset("data/ch0", data=np.random.randint(0, 100, size=(25, 150, 150)))
    ...
    @pytest.mark.parametrize("method", ['background'])
    def test_method(self, method):
        out_file = self.temp_file.with_suffix(f".met.{method}.h5")
        result = self.runner.invoke(
                subtract_delta,
                [self.temp_file.as_posix(), "--output-path", out_file.as_posix(), "--loc-in", "data/ch0", "--loc-out",
                 "df/ch0", "--method", method]
                )
    ...
    def test_overwrite(self, overwrite):
        ...
        result = self.runner.invoke(
                subtract_delta,
                [self.temp_file.as_posix(), "--output-path", out_file.as_posix(), "--loc-in", "data/ch0", "--loc-out",
                 "df/ch0", "--overwrite-first-frame", overwrite]
                )
...
class TestCliViewData:
    ...
    def test_view_data_trace(self):
        ...
        result = self.runner.invoke(
                view_data,
                [self.temp_file.as_posix(), "data/ch0", "--show-trace", True, "--window", "5", "--testing", True]
                )
