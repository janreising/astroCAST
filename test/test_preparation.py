...
from astrocast.preparation import Delta, IO, Input, MotionCorrection


class TestDelta:
    
    @pytest.mark.parametrize("input_type", [".tiff", ".h5", "tiledb"])
    def test_load_save(self, input_type, shape=(25, 100, 100)):
    ...
    @pytest.mark.parametrize("method", ("background", "dFF"))
    def test_methods_run(self, method, shape=(25, 100, 100)):
        
        data = np.random.randint(0, 100, shape, dtype=int)
    ...
    def test_performance_simple(self, shape=(25, 100, 100), noise_level=(50, 3),
        ...
        noise_mean, noise_std = noise_level
        background = np.random.normal(noise_mean, noise_std, shape).astype(int)
        
        # Ensure noise is within a reasonable range to simulate real data
        background = np.clip(background, 0, noise_mean + noise_std * 10)
        
        # Create an artificial signal
        data = np.copy(background)
        for _ in range(10):  # Number of random squares
            # Generate random square coordinates and size
            z = np.random.randint(0, shape[0])
            x = np.random.randint(0, shape[1])
            y = np.random.randint(0, shape[2])
            square_size = int(min([signal_size * x for x in shape]))
            size = np.random.randint(3, square_size)  # Random size of the square
            
            # Ensure the square doesn't exceed the array boundaries
            z_end = min(z + size, shape[0])
            x_end = min(x + size, shape[1])
            y_end = min(y + size, shape[2])
            
            # Add random square with signal intensity values
            signal_level = (signal_multiplier * noise_level[0], signal_multiplier * noise_level[1])
            data[z:z_end, x:x_end, y:y_end] += np.random.normal(*signal_level,
                                                                (z_end - z, x_end - x, y_end - y)).astype(
                    int)
        ...
        if not np.allclose(background, recovered_background, rtol=rtol):
            ...
            raise AssertionError(f"Recovery insufficient {delta_max} > relative {rtol * 100}% "
