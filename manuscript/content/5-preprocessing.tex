...
\added[id=R1, comment=Added warning about risks of denoising.]{The denoising step, while beneficial for reducing noise in imaging data, is optional and may not be appropriate for all types of analyses. This is particularly relevant in studies focusing on fast transient signals (<300 ms), where denoising might obscure crucial biological events \citep{heja_spontaneous_2021,cho_hippocampal_2022,georgiou_ca2_2022}. Users should carefully consider their specific research needs when deciding whether to apply this denoising step. It is advisable to perform a careful examination of the data to ensure that significant biological signals are not inadvertently removed. Additionally, increasing the acquisition speed may further reduce the risk of losing these fast events, as it provides the denoising model with more frames to accurately identify rapid changes.}
