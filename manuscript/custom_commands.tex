...
\definecolor{colorAna}{RGB}{229, 209, 79}
...
\newcommand{\ccomment}[3]{

    % Check the person and assign the color
    \ifstrequal{#1}{Jan}{
        \def\commentcolor{colorJan!50}
        \def\commentauthor{Jan}
    }{}
    \ifstrequal{#1}{Athina}{
        \def\commentcolor{colorAthina!50}
        \def\commentauthor{Athina}
    }{}
    \ifstrequal{#1}{Eric}{
        \def\commentcolor{colorEric!50}
        \def\commentauthor{Eric}
    }{}
    \ifstrequal{#1}{Ana}{
        \def\commentcolor{colorAna!50}
        \def\commentauthor{Ana}
    }{}

    \colorbox{\commentcolor}{#2}
    \pdfmargincomment[icon=Note, color=\commentcolor, author=\commentauthor]{#3}
}

%\newcommand{\lbash}[1]{\lstset{frame=single} \begin{lstlisting}[language=bash] #1 \end{lstlisting}}

%\newcommand{\fref}[1]{ (Figure~\ref{fig:#1})}
\NewDocumentCommand{\fref}{ m O{} }{ (Figure~\ref{fig:#1}\IfValueT{#2}{\,#2})}
\NewDocumentCommand{\frefsupp}{ m O{} }{ (Supplement~\ref{supp-fig:#1}\IfValueT{#2}{\,#2})}
