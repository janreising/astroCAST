...
class IO:
    ...
    def __init__(self):
        ...
        # Introspect the class and get all '_load_*' methods
        self.load_methods = {}
        for name, method in inspect.getmembers(self, predicate=lambda x: inspect.isfunction(x) or inspect.ismethod(x)):
            if name.startswith('_load_'):
                
                suffix = "." + name.replace('_load_', '')
                
                self.load_methods[suffix] = method
                self.load_methods[suffix.upper()] = method
        
        # Introspect the class and get all '_save_*' methods
        self.save_methods = {}
        for name, method in inspect.getmembers(self, predicate=inspect.ismethod):
            if name.startswith('_save_'):
                
                suffix = "." + name.replace('_save_', '')
                
                self.save_methods[suffix] = method
                self.save_methods[suffix.upper()] = method
    
    def get_loader_function(self, suffix: str, kwarg_arguments: dict):
        
        if suffix not in self.load_methods:
            raise KeyError(
                    f"File format {suffix} is not implemented. Choose on of {list(self.load_methods.keys())}")
        
        load_method = self.load_methods[suffix]
        loader_params = inspect.signature(load_method).parameters
        
        # Filter the kwargs to only include what the loader function needs
        loader_kwargs = {k: v for k, v in kwarg_arguments.items() if k in loader_params}
        
        return lambda x: load_method(x, **loader_kwargs)
    
    def get_saver_function(self, suffix: str, kwarg_arguments: dict):
        
        if suffix not in self.save_methods:
            raise KeyError(
                    f"File format {suffix} is not implemented. Choose on of {list(self.save_methods.keys())}")
        
        save_method = self.save_methods[suffix]
        saver_params = inspect.signature(save_method).parameters
        
        # Filter the kwargs to only include what the loader function needs
        saver_kwargs = {k: v for k, v in kwarg_arguments.items() if k in saver_params}
        
        return lambda path, data: save_method(path=path, data=data, **saver_kwargs)
    
    def load(self, path, h5_loc=None, sep: str = "_", z_slice: Tuple[int, int] = None, lazy: bool = False,
             chunks: Union[Tuple, str] = "auto", alpha_numerical_sorting: bool = True):
        ...
        Args:
            ...
            z_slice: Z axis frames to load.
            lazy: Load data on demand (lazy) or to memory.
            chunks: Chunks to save if the file format has that capability.
            alpha_numerical_sorting: Use sorting that is intuitive to humans.
        ...
        # check arguments
        kwarg_arguments = {'lazy': lazy, 'chunks': chunks, 'h5_loc': h5_loc}
        
        if z_slice is not None and (not isinstance(z_slice, (tuple, list)) or len(z_slice) != 2):
        ...
        if isinstance(path, Path) and path.is_dir():
            ...
            files = [f for f in path.glob("*") if f.suffix in self.load_methods.keys()]
            
            if len(files) < 1:
                raise FileNotFoundError(f"Couldn't find files in directory {path} with "
                                        f"suffix in {self.load_methods.keys()}")
            
            # ensure singular file type and get loader function
            suffixes = set([f.suffix for f in files])
            if len(suffixes) > 1:
                raise TypeError(f"Folder contains multiple file types: {suffixes}")
            
            suffix = files[0].suffix
            load_function = self.get_loader_function(suffix, kwarg_arguments)
            
            # sort alpha numerically
            if alpha_numerical_sorting:
                files = self.sort_alpha_numerical_names(files, sep=sep)
            
            # Read files using dask.array and stack them
            data = da.stack([load_function(f) for f in files])
            data = np.squeeze(data)
            
            if len(data.shape) != 3:
                logging.warning(f"Expected data dimension to be 3D instead of {data.shape}."
                                f"Pipeline might not be able to handle this type of data.")
        
        elif isinstance(path, Path) and path.is_file():
            
            # get suffix and appropriate loader function
            suffix = path.suffix
            load_function = self.get_loader_function(suffix, kwarg_arguments)
    ...
    @staticmethod
    def _load_npy(path, lazy=False, chunks="auto"):
    ...
    @staticmethod
    def _load_tdb(path, lazy=False, chunks="auto"):
    ...
    @staticmethod
    def _load_h5(path, h5_loc, lazy=False, chunks="auto"):
    ...
    def _load_hdf5(self, path, h5_loc, lazy=False, chunks="auto"):
        return self._load_h5(path, h5_loc, lazy=lazy, chunks=chunks)
    
    @staticmethod
    def _load_csv(path, chunks="auto"):
    ...
    @staticmethod
    def _load_tiff(path, lazy=False):
    ...
    def _load_tif(self, path, lazy=False):
        return self._load_tiff(path, lazy=lazy)
    
    @staticmethod
    def _helper_load_images(path: Path, lazy: bool = False):
        data = imageio.v2.imread(path.as_posix())
    ...
    def _load_png(self, path, lazy=False):
        return self._helper_load_images(path=path, lazy=lazy)
    
    def _load_jpg(self, path, lazy=False):
        return self._helper_load_images(path=path, lazy=lazy)
    
    def _load_jpeg(self, path, lazy=False):
        return self._helper_load_images(path=path, lazy=lazy)
    
    @staticmethod
    def _infer_chunks(data: Union[np.ndarray, da.Array]):
    ...
    @staticmethod
    def _infer_compression(data: Union[np.ndarray, da.Array]):
    ...
    def save(self, path, data, h5_loc=None, chunks=None, compression=None, fps=16, z_slice: Tuple[int, int] = None,
             overwrite=False):
        ...
        Args:
            ...
            fps: frames per second when saving to .avi
            z_slice: Z axis frames to select for saving
        ...
        # check arguments
        kwarg_arguments = dict(h5_loc=h5_loc, chunks=chunks, compression=compression, fps=fps, overwrite=overwrite)
        ...
        for key, item in data.items():
            if not isinstance(item, (np.ndarray, da.Array)):
        ...
        for k in data.keys():
            ...
            if chunks == "infer":
                chunks = self._infer_chunks(channel_data)
            
            if compression == "infer":
                compression = self._infer_compression(channel_data)
            
            if z_slice is not None:
                z0, z1 = z_slice
                channel_data = channel_data[z0:z1]
            
            kwarg_arguments["channel"] = k
            save_function = self.get_saver_function(path.suffix, kwarg_arguments)
            save_path = save_function(path, channel_data)
            
            logging.info(f"dataset saved to {save_path}::{k}")
            saved_paths.append(save_path)
        
        return saved_paths if len(saved_paths) > 1 else saved_paths[0]  # Return the list of saved file paths
    
    def _save_h5(self, path: Path, h5_loc: Union[dict, str, None], data: Union[np.ndarray, da.Array],
                 channel: str, compression: str, chunks: Tuple = None, overwrite: bool = False):
        ...
        if isinstance(h5_loc, dict):
            ...
            loc = channel if "/" in str(channel) else f"io/{channel}"
        
        elif len(data) == 1:
            loc = f"{h5_loc}/{channel}" if "/" not in h5_loc[:-1] else h5_loc
        
        else:
            loc = f"{h5_loc}/{channel}"
        
        self.exists_and_clean(path, h5_loc=loc, overwrite=overwrite)
        logging.info(f"saving channel {channel} to '{loc}'")
        
        if isinstance(channel, da.Array):
            # Save Dask array
            with ProgressBar(minimum=10, dt=1):
                da.to_hdf5(path, loc, channel, chunks=chunks, compression=compression, shuffle=False)
        
        else:
            # Save NumPy array
            with h5py.File(path, "a") as f:
                f.create_dataset(loc, data=data, shape=data.shape, dtype=data.dtype,
                                 chunks=chunks, compression=compression, shuffle=False)
        
        return path
    
    def _save_hdf5(self, path: Path, h5_loc: Union[dict, str, None], data: Union[np.ndarray, da.Array],
                   channel: str, compression: str, chunks: Tuple = None, overwrite: bool = False):
        return self._save_h5(path=path, h5_loc=h5_loc, data=data, channel=channel,
                             compression=compression, chunks=chunks, overwrite=overwrite)
    
    def _save_tdb(self, path: Path, data: Union[np.ndarray, da.Array], channel: str, chunks: Tuple = None,
                  overwrite: bool = False):
        
        if isinstance(data, np.ndarray):
            data = da.from_array(data, chunks=chunks if chunks is not None else "auto")
        
        fpath = path.with_suffix(f".{channel}.tdb") if len(data.keys()) > 1 else path
        self.exists_and_clean(fpath, overwrite=overwrite)
        with ProgressBar(minimum=10, dt=1):
            da.to_tiledb(data, fpath.as_posix(), compute=True)
        
        return fpath
    
    def _save_tif(self, path: Path, data: Union[np.ndarray, da.Array], channel: str, overwrite: bool = False):
        fpath = path.with_suffix(f".{channel}.tiff")
        self.exists_and_clean(fpath, overwrite=overwrite)
        
        tifffile.imwrite(fpath, data=data)
        return fpath
    
    def _save_tiff(self, path: Path, data: Union[np.ndarray, da.Array], channel: str, overwrite: bool = False):
        return self._save_tif(path=path, data=data, channel=channel, overwrite=overwrite)
    
    def _save_czi(self, path: Path):
        raise NotImplementedError("currently we are not aware that python can save images in .czi format.")
    
    def _save_npy(self, path: Path, data: Union[np.ndarray, da.Array], channel: str, overwrite: bool = False):
        
        fpath = path.with_suffix(f".{channel}.npy")
        self.exists_and_clean(fpath, overwrite=overwrite)
        
        if isinstance(data, np.ndarray):
            np.save(file=fpath.as_posix(), arr=data)
        
        else:
            with ProgressBar(minimum=10, dt=1):
                da.to_npy_stack(fpath, x=data, axis=0)
        
        return fpath
    
    def _save_avi(self, path: Path, data: Union[np.ndarray, da.Array], channel: str, fps: int = 16,
                  overwrite: bool = False):
        
        import cv2
        
        fpath = path.with_suffix(f".{channel}.avi")
        self.exists_and_clean(fpath, overwrite=overwrite)
        
        # Get the dimensions of the numpy array
        frames, X, Y = data.shape[:3]
        
        # Define the codec and create a VideoWriter object
        out = cv2.VideoWriter(fpath.as_posix(), fourcc=cv2.VideoWriter_fourcc('M', 'J', 'P', 'G'),
                              fps=fps, frameSize=(X, Y), isColor=True)
        ...
        for i in tqdm(range(frames)):
            ...
            if len(frame.shape) == 3 and frame.shape[2] == 4:
                # Convert BGRA to BGR (discard alpha channel)
    ...
    @staticmethod
    def exists_and_clean(path, h5_loc="", overwrite=False):
...
class MotionCorrection:
    ...
    def clean_up(self):
        ...
        if self.working_directory is not None:
            temp_h5_path = Path(self.working_directory.name) if isinstance(self.working_directory,
                                                                           tempfile.TemporaryDirectory) else Path(
    ...
    def save(self, output=None, h5_loc="mc", chunks=None, compression=None, remove_intermediate=True):
        ...
        if output is None:
            ...
            if len(split_h5) < 2:
                ...
                raise ValueError(
                        f"please provide h5_loc as 'channel_name' or 'folder/channel_name' instead of {h5_loc}")
...
class Delta:
    ...
    def run(self, window, method="background", processing_chunks="infer", output_path=None,
        ...
        data = self.prepare_data(self.input_, h5_loc=self.loc, chunks=processing_chunks, output_path=output_path,
                                 lazy=lazy)
        ...
        # Parallel from .tdb file
            ...
            def wrapper(path, ranges):
                ...
                with tiledb.open(path, mode="r") as tdb:
                    ...
                    res = calculate_delta_min_filter(data, window, method=method,
                                                     inplace=False)  # TODO does this make sense?
